...
function voisins_visitables(som::Tuple{Int64,Int64},A::Matrix,Visited::Matrix) # Prend les coordonnées d'un sommet et renvoi la liste de ses voisins visitables dans A
  a=[]             
...
function model_Dijkstra(A::Matrix{Char}, start::Tuple{Int64,Int64}, final::Tuple{Int64,Int64} )
    ...
    Pred::Matrix{Tuple{Int64,Int64}}= Matrix(undef , size(A,1),size(A,2)) #Matrice : liste des prédécesseurs de chaque sommet     
    ...
    while (t!=final) && (!isempty(pq))
      t=dequeue!(pq)
      v=voisins_visitables(t,A,Visited)
      Visited[t[1],t[2]]=1
      cpt=cpt+1
      ...
      if v==[]
        ...
        for i in 1:size(v,1)
          Open[v[i][1],v[i][2]]=1
            ...
            push!(pq,v[i]=>Dist_to_start[v[i][1],v[i][2]])
        end 
      end
...
function model_Astar(A::Matrix{Char}, start::Tuple{Int64,Int64}, final::Tuple{Int64,Int64} )
...
function Astar(nom_fichier,start::Tuple{Int64,Int64},final::Tuple{Int64,Int64})

   A=Extract_M(nom_fichier)
   @time B=model_Astar(A,start,final)
...
function Dijkstra(nom_fichier,start::Tuple{Int64,Int64},final::Tuple{Int64,Int64})

    A=Extract_M(nom_fichier)
    @time B=model_Dijkstra(A,start,final)
...
function A_star(A::Matrix{Int64}, t::Vector{Int64})
